...
_CMAKE_VAR_REGEX = re.compile(r"\100([A-Za-z_0-9]*)\100")
_CMAKE_VAR_REGEX_1 = re.compile(r"\s*#undef\s+([A-Za-z_0-9]*)(\s.*)?$")
...
def _expand_cmakedefines(line, cmake_vars):
    ...
    match = _CMAKE_VAR_REGEX_1.match(line)
    if match:
        name = match.group(1)
        suffix = match.group(2) or ""
        # print(name)
        if name in cmake_vars:
            return "#define {} {}\n".format(name,cmake_vars.get(name,"0"))
...
def main():
    ...
    # with open("/home/ysahil/Academics/test_output.txt","w") as f:
    for line in sys.stdin:
        sys.stdout.write(_expand_cmakedefines(line, cmake_vars))
            # f.write(_expand_cmakedefines(line,cmake_vars))
    # f.close()
